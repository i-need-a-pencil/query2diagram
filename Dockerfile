...
COPY ./requirements.txt /codexp
RUN apt-get remove python3-blinker -y
RUN --mount=type=cache,target=/root/.cache \
    python${PYTHON_VERSION} -m pip install -r requirements.txt

COPY ./codexp /codexp/codexp
COPY ./setup.py /codexp
COPY ./pyproject.toml /codexp
COPY ./scripts /codexp/scripts
RUN chmod +x /codexp/scripts/start.sh
RUN python${PYTHON_VERSION} -m pip install -e .


CMD /codexp/scripts/start.sh
EXPOSE 22
