...
def _jaccard(query_feat: np.ndarray, candidate_feat: np.ndarray) -> float:
    ...
    return float(intersect_size) / float(len(query_feat) + len(candidate_feat) - intersect_size)
...
@njit(nogil=True)
def _jaccard_overlap_index_threshold(n_elements: int, similarity_threshold: float) -> int:
    """
    It is a lower bound of `|a ⋂ b|` for any `b` which _jaccard(a, b) >= similarity_threshold
...
def _jaccard_minoverlap(a_size: int, b_size: int, similarity_threshold: float) -> float:
    ...
    """
    return float64((similarity_threshold / (1.0 + similarity_threshold)) * (a_size + b_size))
...
@njit(nogil=True)
def _cosine_overlap_index_threshold(n_elements: int, similarity_threshold: float) -> int:
    """
    It is a lower bound of `|a ⋂ b|` for any `b` which _cosine(a, b) >= similarity_threshold
    This lower bound is used in creating prefix while indexing data
    :param n_elements: size of `a` features
...
@njit(nogil=True)
def _containment_min_overlap_threshold(n_elements: int, similarity_threshold: float) -> int:
    """
...
@njit(nogil=True)
def _containment_min_overlap_index_threshold(n_elements: int, similarity_threshold: float) -> int:
    """
    It is a lower bound of `|a ⋂ b|` for any `b` which _containment_min(a, b) >= similarity_threshold
    This lower bound is used in creating prefix while indexing data
    :param n_elements: size of `a` features
...
@njit(nogil=True)
def _containment_min_minoverlap(a_size: int, b_size: int, similarity_threshold: float) -> float:
    """
...
@njit(nogil=True)
def _containment_overlap_index_threshold(n_elements: int, similarity_threshold: float) -> int:  # noqa: U100
    """
...
@njit(nogil=True)
def _containment_minoverlap(a_size: int, b_size: int, similarity_threshold: float) -> float:  # noqa: U100
    """
    It is an equivalent threshold for `|a ⋂ b|`, i.e. |a ⋂ b| >= minoverlap <=>
...
def _create_get_prefix_size_query(overlap_threshold_func: Callable) -> Callable:
    ...
    def _get_prefix_size_query(n_elements: int, similarity_threshold: float) -> int:
        ...
        :return:
        """
        overlap_threshold = overlap_threshold_func(n_elements=n_elements, similarity_threshold=similarity_threshold)
        return n_elements - overlap_threshold + 1

    return _get_prefix_size_query


_jaccard_get_prefix_size_query = _create_get_prefix_size_query(_jaccard_overlap_threshold)
_cosine_get_prefix_size_query = _create_get_prefix_size_query(_cosine_overlap_threshold)
_containment_min_get_prefix_size_query = _create_get_prefix_size_query(_containment_min_overlap_threshold)
_containment_get_prefix_size_query = _create_get_prefix_size_query(_containment_overlap_threshold)


def _create_position_filter(minoverlap_func: Callable) -> Callable:
    @njit(nogil=True)
    def _position_filter(
        a_size: int, b_size: int, a_prefix_pos: int, b_prefix_pos: int, similarity_threshold: float
    ) -> bool:
        """
        ...
        equal_condition = is_close(min_value, minoverlap_value, rel_tol=1e-09, abs_tol=0.0)
...
def _create_verify_pair(similarity_function: Callable) -> Callable:
    @njit(nogil=True)
    def _verify_pair(query_feat: np.ndarray, candidate_feat: np.ndarray, similarity_threshold: float) -> bool:
        """
...
def _create_calculate_index_size(overlap_index_threshold_func: Callable) -> Callable:
    ...
    def calculate_index_size(pointers: np.ndarray, prefix_sizes: np.ndarray, similarity_threshold: float) -> int:
        """
...
jaccard_calculate_index_size = _create_calculate_index_size(_jaccard_overlap_index_threshold)
cosine_calculate_index_size = _create_calculate_index_size(_cosine_overlap_index_threshold)
containment_min_calculate_index_size = _create_calculate_index_size(_containment_min_overlap_index_threshold)
containment_calculate_index_size = _create_calculate_index_size(_containment_overlap_index_threshold)


# TODO: make documentation for SSS with pictures
@njit(nogil=True)
def _calc_token_index_size(
    token_index_size: np.ndarray,
    current_token_pos: np.ndarray,
    input_pointer: np.ndarray,
    input_data: np.ndarray,
    prefix_sizes: np.ndarray,
) -> None:
    """
    Calculate start and end positions for each token in index
    :param token_index_size: empty matrix (n_tokens, 2) which will be filled
    :param current_token_pos: empty array (n_tokens,) which will maintain position for each token in index
    ...
    for idx, (start, end) in enumerate(input_pointer):
        ...
        for token_id in prefix:
            current_token_pos[token_id] += 1

    token_index_size_counter = 0
    for idx in range(token_index_size.shape[0]):
        token_index_size[idx, 0] = token_index_size_counter
        token_index_size_counter += current_token_pos[idx]
        token_index_size[idx, 1] = token_index_size_counter

    for idx in range(current_token_pos.shape[0]):
        current_token_pos[idx] = token_index_size[idx, 0]
...
def fill_index(
    ...
    :param current_token_pos: array which maintains current position for each token in index
    :param prefix_sizes: size of prefix for each feature vector
    :param index: empty matrix (index_size, 2) which will be filled
    :param token_index_size: matrix with start and end positions for each token in index
    :param input_pointer: matrix with start and end positions of features in input_data for each vector
    :param input_data: features of all vectors
    """
    _calc_token_index_size(
        token_index_size=token_index_size,
    ...
    for feature_index, (start, end) in enumerate(input_pointer):
        ...
        prefix = features[:prefix_size]
        for token_prefix_pos, token_id in enumerate(prefix):
            current_token_idx = current_token_pos[token_id]
            index[current_token_idx, 0] = feature_index
            index[current_token_idx, 1] = token_prefix_pos
            current_token_pos[token_id] += 1
...
def _n_unique(data: np.ndarray) -> int:
    """
    Calculate number of unique integer tokens
    :param data: integer features of all vectors
    :return: number of unique integer tokens
    """
...
def _fill_counter(counter: np.ndarray, data: np.ndarray) -> None:
    """
    Calculate amount for each integer token
    :param counter: empty array (n_tokens,) which will be filled
    :param data: integer features of all vectors
    """
    for token in data:
        counter[token] += 1
...
def _create_frequency_order_mapping(counter: np.ndarray) -> np.ndarray:
    """
    Calculate token2id mapping due to frequency order
    :param counter: array with amounts for each integer token
    :return: array mapping integer token to its id
    """
    id2token = np.argsort(counter)
    token2id = np.zeros_like(id2token)
    for idx in range(len(id2token)):
        token2id[id2token[idx]] = idx
    return token2id


@njit(nogil=True)
def _apply_frequency_order_transform(
    data: np.ndarray, pointers: np.ndarray, token2id: np.ndarray, fot_data: np.ndarray
) -> None:
    """
    Apply frequency order transformation to integer data using token2id - frequency order mapping
    ...
    :param token2id: mapping integer tokens from data to its ids due to frequency order
    :param fot_data: array shaped like data where frequency order transformed data will be written to
    """
    unknown_token_id = len(token2id)
    for pointer in pointers:
        start, end = pointer
        data_to_transform = data[start:end]
        for idx in range(len(data_to_transform)):
            token = data_to_transform[idx]
            if token >= len(token2id):
                data_to_transform[idx] = unknown_token_id
            else:
                data_to_transform[idx] = token2id[token]
...
def frequency_order_transform_from_flattened_format(
    ...
    :return: frequency order transformed data, start and end positions for data, mapping integer tokens to ids due to
    frequency order
    """
    n_tokens = _n_unique(data=data)
    token2freq = np.zeros((n_tokens,), dtype=int)
    _fill_counter(counter=token2freq, data=data)
    token2id = _create_frequency_order_mapping(token2freq)
    _apply_frequency_order_transform(data=data, pointers=pointers, token2id=token2id, fot_data=data)
    return data, pointers, token2id
...
def frequency_order_transform(
    ...
    data, pointers = _to_flattened_format(
        dataset=index_dataset, features_lengths=features_lengths, mmap=mmap, mmap_filename="index_data_mmap"
    )
    return frequency_order_transform_from_flattened_format(data, pointers)


def frequency_order_transform_queries(
    queries_dataset: Sequence[np.ndarray], token2id: np.ndarray
) -> Tuple[np.ndarray, np.ndarray]:
    """
    Apply frequency order transformation for numeric transformed query dataset using token2id - mapping calculated while
    frequency order transforming dataset for indexing
    :param queries_dataset: sequence of integer features of query vectors
    :param token2id: mapping integer tokens to ids for already indexed data
    :return: frequency order transformed query data, start and end positions for query data
    """
    features_lengths = np.array([len(features) for features in queries_dataset], dtype=np.int32)
    queries_data, pointers = _to_flattened_format(queries_dataset, features_lengths=features_lengths)
    _apply_frequency_order_transform(token2id=token2id, data=queries_data, pointers=pointers, fot_data=queries_data)
    return queries_data, pointers


def _to_flattened_format(
    dataset: Iterable[np.ndarray], features_lengths: np.ndarray, mmap: bool = False, mmap_filename: str = None
) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    logging.debug(f"To_flattened_format Starting: memory usage is {process.memory_info().rss / 1024**2}")
    if mmap and mmap_filename is None:
        raise ValueError("If mmap flag is True then mmap_filename argument should be provided")
    if mmap:
        data = np.memmap(filename=mmap_filename, dtype=np.int32, mode="w+", shape=(np.sum(features_lengths),))
    else:
    ...
    logging.debug(f"To_flattened_format Finished: memory usage is {process.memory_info().rss / 1024 ** 2}")
...
symmetric_similarity_funcs = ["jaccard", "cosine", "containment_min"]
asymmetric_similarity_funcs = ["containment"]
...
def query(
    ...
    :param features: frequency order transformed feature vector which will be in query
    ...
    token_index_size = np.asarray(token_index_size)
    index_data = np.asarray(index_data)
    index_pointers = np.asarray(index_pointers)

    get_prefix_size_query_func = _get_prefix_size_query_funcs[similarity_func_name]
    position_filter_func = _position_filter_funcs[similarity_func_name]
    verify_pair_func = _verify_pair_funcs[similarity_func_name]
    similarity_func = similarity_funcs[similarity_func_name]

    result = set()
    prefix_size = get_prefix_size_query_func(n_elements=len(features), similarity_threshold=similarity_threshold)
    prefix = features[:prefix_size]

    features_len = len(features)
    candidates_to_check = 0
    for token_id_pos in range(len(prefix)):
        token_id = prefix[token_id_pos]
        if token_id >= token_index_size.shape[0]:
            continue
        index_start = token_index_size[token_id, 0]
        index_end = token_index_size[token_id, 1]
        index_to_check = index[index_start:index_end]
        for idx in range(len(index_to_check)):
            candidate_idx = index_to_check[idx, 0]
            candidate_token_id_pos = index_to_check[idx, 1]
            ...
            if position_filter_func(
                ...
                a_prefix_pos=token_id_pos,
                b_prefix_pos=candidate_token_id_pos,
                similarity_threshold=similarity_threshold,
            ) and verify_pair_func(
                query_feat=features, candidate_feat=candidate_features, similarity_threshold=similarity_threshold
            ):
                similarity_score = similarity_func(features, candidate_features)
                result.add((candidate_idx, similarity_score))

    logging.debug(f"Candidates to check {candidates_to_check}\nResults len for this one query {len(result)}")
    return result
...
def query_batch(
    ...
    def job_task(
        ...
        results = []
        for idx in tqdm(query_range, "Querying", disable=not show_progress, leave=False):
            query_res = query(
                similarity_func_name=similarity_func_name,
                features=query_data_shared[query_pointers_shared[idx, 0] : query_pointers_shared[idx, 1]],
                index=index_shared,
                token_index_size=token_index_size_shared,
    ...
    def split_work(process_num: int):
        ...
        query_data_shared = query_data if type(query_data) is np.memmap else sharedmem.full_like(query_data, query_data)
        query_pointers_shared = (
            query_pointers if type(query_pointers) is np.memmap else sharedmem.full_like(query_pointers, query_pointers)
        )
        index_shared = index if type(index) is np.memmap else sharedmem.full_like(index, index)
        index_data_shared = index_data if type(index_data) is np.memmap else sharedmem.full_like(index_data, index_data)
        token_index_size_shared = (
            token_index_size
            if type(token_index_size) is np.memmap
            else sharedmem.full_like(token_index_size, token_index_size)
        )
        index_pointers_shared = (
            index_pointers if type(index_pointers) is np.memmap else sharedmem.full_like(index_pointers, index_pointers)
        )
...
_intersect(a=np.array([0, 1, 1, 2], dtype=np.int32), b=np.array([1, 1, 2], dtype=np.int32))

_jaccard(query_feat=np.array([0, 1, 1, 2], dtype=np.int32), candidate_feat=np.array([1, 1, 2], dtype=np.int32))
_cosine(query_feat=np.array([0, 1, 1, 2], dtype=np.int32), candidate_feat=np.array([1, 1, 2], dtype=np.int32))
_containment_min(query_feat=np.array([0, 1, 1, 2], dtype=np.int32), candidate_feat=np.array([1, 1, 2], dtype=np.int32))
_containment(query_feat=np.array([0, 1, 1, 2], dtype=np.int32), candidate_feat=np.array([1, 1, 2], dtype=np.int32))
...
_jaccard_position_filter(a_size=12, b_size=14, a_prefix_pos=2, b_prefix_pos=3, similarity_threshold=0.8)
_cosine_position_filter(a_size=12, b_size=14, a_prefix_pos=2, b_prefix_pos=3, similarity_threshold=0.8)
_containment_min_position_filter(a_size=12, b_size=14, a_prefix_pos=2, b_prefix_pos=3, similarity_threshold=0.8)
_containment_position_filter(a_size=12, b_size=14, a_prefix_pos=2, b_prefix_pos=3, similarity_threshold=0.8)
...
_calc_token_index_size(
    token_index_size=np.zeros((5, 2), dtype=np.int32),
    current_token_pos=np.zeros((5,), dtype=np.int32),
...
fill_index(
    ...
    token_index_size=np.zeros((8, 2), dtype=np.int32),
    current_token_pos=np.zeros((8,), dtype=np.int32),
    input_pointer=np.array([[0, 7], [7, 14], [14, 19]], dtype=np.int32),
    input_data=np.array([0, 3, 4, 6, 6, 7, 7, 1, 3, 4, 5, 5, 7, 7, 2, 6, 6, 6, 7], dtype=np.int32),
    prefix_sizes=np.array([3, 3, 2], dtype=np.int32),
)

_n_unique(data=np.array([1, 2, 2, 1], dtype=np.int32))

_fill_counter(counter=np.zeros((2,), dtype=np.int32), data=np.array([1, 2, 2, 1], dtype=np.int32))
...
_apply_frequency_order_transform(
    ...
    token2id=np.array([0, 1, 5, 2, 3, 4], dtype=np.int32),
